...
static char *searchInside(const char *dir, const char *filename)
{
	...
	while ((ep = readdir(dp)) != NULL) {
		...
		if ((int)strlen(ep->d_name) == len
		    && !strcmp(ep->d_name, filename)) {
			(void)closedir(dp);
			return curfile;
		}
		lstat(curfile, &sb);
...
static int linux_set_rate(struct wif *wi, int rate)
{
	...
	case DT_MADWIFING:
		...
		strncpy(ifr.ifr_name, wi_get_ifname(wi),
	...
	if (dev->arptype_in == ARPHRD_IEEE80211_PRISM) {
		...
		if (tmpbuf[7] == 0x40) {
			...
			if (ri) {
				...
				ri->ri_noise =
				    *(unsigned int *)(tmpbuf + 0x33 + 12);
				ri->ri_rate =
				    (*(unsigned int *)(tmpbuf + 0x33 + 24)) *
				    500000;
			...
			if (ri) {
				ri->ri_mactime =
				    *(u_int64_t *) (tmpbuf + 0x5C - 48);
				ri->ri_channel =
				    *(unsigned int *)(tmpbuf + 0x5C - 36);
				ri->ri_power = *(unsigned int *)(tmpbuf + 0x5C);
				ri->ri_noise =
				    *(unsigned int *)(tmpbuf + 0x5C + 12);
				ri->ri_rate =
				    (*(unsigned int *)(tmpbuf + 0x5C + 24)) *
				    500000;
...
//                if( ! memcmp( iface[i], "ath", 3 ) )
	...
	if (fcs_removed == 0
	    && check_crc_buf_osdep(tmpbuf + n, caplen - 4) == 1) {
		caplen -= 4;
	}
...
static int opensysfs(struct priv_linux *dev, char *iface, int fd)
{
	...
	/* bcm43xx injection */
	if (fd2 == -1)
		snprintf(buf, 256, "/sys/class/net/%s/device/inject_nofcs",
			 iface);
	fd2 = open(buf, O_WRONLY);
...
static int do_linux_open(struct wif *wi, char *iface)
{
	...
	if (strlen(iface) == 5 && memcmp(iface, "wlan", 4) == 0) {
		...
		snprintf(strbuf, sizeof(strbuf) - 1,
			 ...
			 "grep p2CnfWEPFlags >/dev/null", iface);

		if (system(strbuf) == 0) {
			if (uname(&checklinuxversion) >= 0) {
				/* uname succeeded */
				if (strncmp
				    (checklinuxversion.release, "2.6.", 4) == 0
				    && strncasecmp(checklinuxversion.sysname,
						   "linux", 5) == 0) {
					/* Linux kernel 2.6 */
					kver =
					    atoi(checklinuxversion.release + 4);
		...
		snprintf(strbuf, sizeof(strbuf) - 1,
			 "iwpriv %s 2>/dev/null | "
			 "grep  GetAcx111Info  >/dev/null", iface);
	...
	if (strcmp(iface, "ra0") == 0 ||
		...
		memset(strbuf, 0, sizeof(strbuf));
		snprintf(strbuf, sizeof(strbuf) - 1,
			 "iwpriv %s rfmontx 1 >/dev/null 2>/dev/null", iface);
		system(strbuf);
	}
	...
	if ((strlen(iface) >= 4 || strlen(iface) <= 6)
		...
		if (f != NULL) {
			...
			sprintf((char *)strbuf, "/proc/sys/net/%s/dev_type",
				iface);
	...
	if (memcmp(iface, "eth", 3) == 0) {
		...
		snprintf(strbuf, sizeof(strbuf) - 1,
			 "iwpriv %s 2>/dev/null | "
			 "grep get_scan_times >/dev/null", iface);
	...
	if (memcmp(iface, "eth", 3) == 0) {
		if ((pid = fork()) == 0) {
			...
			close(2);
			chdir("/");
			execlp("iwpriv", "iwpriv", iface, "get_regdomain",
			       NULL);
			exit(1);
		}
	...
	if (openraw(dev, iface, dev->fd_out, &dev->arptype_out, dev->pl_mac) !=
	    0) {
		goto close_out;
	}
