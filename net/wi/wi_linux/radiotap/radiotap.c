...
int ieee80211_radiotap_iterator_init(struct ieee80211_radiotap_iterator
				     ...
				     *radiotap_header, int max_length, const struct
				     ieee80211_radiotap_vendor_namespaces *vns)
{
...
static void
find_ns(struct ieee80211_radiotap_iterator *iterator,
...
#ifdef RADIOTAP_SUPPORT_OVERRIDES
static int
find_override(struct ieee80211_radiotap_iterator *iterator,
	      int *align, int *size)
{
	int i;
	...
	while (1) {
		...
		default:
			...
			if (!iterator->current_namespace ||
				...
				align =
				    iterator->current_namespace->
				    align_size[iterator->_arg_index].align;
				size =
				    iterator->current_namespace->
				    align_size[iterator->_arg_index].size;
			}
			if (!align) {
		...
		if (iterator->_arg_index % 32 ==
			...
			oui = (*iterator->_arg << 16) |
			    (*(iterator->_arg + 1) << 8) | *(iterator->_arg +
							     2);
