...
/*
	use below to redirect standard streams.
...
uint8_t hex2int(unsigned char c)
{
...
{
	int hex_len;
	if ((hex_len = strlen(hex)) % 2 != 0) {
		return -1;
	}

	int i;
	for (i = 0; i < hex_len; i += 2) {
		byte[i >> 1] = (hex2int(hex[i]) << 4) + hex2int(hex[i + 1]);
	}
